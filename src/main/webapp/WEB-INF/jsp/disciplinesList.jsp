...
<html lang="ru">
 ...
 <h1 style="text-align: center; margin-top: 30px">
      Список дисциплин для специальности ${speciality.title}
    </h1>
    <section
      style="
        margin: 30px auto 0;
    ...
    >
          <table border="1" cellpadding="5">
             <tr>
      <th style="width: 285px">Название дисциплины</th>
      <th style="width: 185px">Форма аттестации</th>
      <tr>
      <c:forEach items="${speciality.disciplines}" var="d">
        <tr>
             <td>${d.name}</td>
            <td> <c:choose>
                     <c:when test="${d.type==1}">экзамен</c:when>
        <c:when test="${d.type == 2}">зачет</c:when>
        <c:when test="${d.type == 3}">дифференцированный зачет</c:when>
         <c:when test="${d.type == 4}">курсовой проект</c:when>
                 </c:choose></td>
        </tr>
      </c:forEach> >    
          </table>
